...
scene_objects = [
    ...
    Triangle(point0=np.array([-0.25, 0.25, 0.25]),
...
def find_nearest_object_to_ray(ray):
...
def main():
    ...
    for i, y in enumerate(np.linspace(screen.top, screen.bottom, screen.height)):
        for j, x in enumerate(np.linspace(screen.left, screen.right, screen.width)):
            ...
            nearest_obj_distance, nearest_obj = find_nearest_object_to_ray(ray)
            ...
            for light in scene_lights:
                ...
                light_ray_intersection_distance, _ = find_nearest_object_to_ray(light_ray)
                ...
                # diffuse
                illumination += nearest_obj.material.diffuse * light.material.diffuse * np.dot(light_ray.direction,
                                                                                               intersection_normal)
                ...
                illumination += nearest_obj.material.specular * light.material.specular * np.dot(
                    intersection_normal, H) ** (nearest_obj.material.shininess / 4)
